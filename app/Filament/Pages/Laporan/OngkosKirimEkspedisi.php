...
class OngkosKirimEkspedisi extends Page implements \Filament\Actions\Contracts\HasActions
{
    ...
    {
        if (in_array($propertyName, ['dateMode', 'startDate', 'endDate', 'perPage', 'search'])) {
            $this->resetPage();
            ...
            \Filament\Actions\Action::make('filter')
                ...
                ->form([
                    ...
                    \Filament\Forms\Components\DatePicker::make('endDate')
                        ...
                        ->required(),
                    \Filament\Forms\Components\Select::make('dateMode')
                        ->hiddenLabel()
                        ->options([
                            'transaksi' => 'Tanggal Transaksi',
                            'pengiriman' => 'Tanggal Pengiriman',
                        ])
                        ->default($this->dateMode)
                        ->required(),
                ])
    ...
    public function getViewData(): array
    {
        ...
        if ($this->search) {
            $query->where(DB::raw('COALESCE(sm.name, "Tanpa Ekspedisi")'), 'like', "%{$this->search}%");
        }
        ...
        $totalGroups = DB::table(DB::raw("({$query->toSql()}) as sub"))->mergeBindings($query)->count();
        $perPageCount = $this->perPage === 'all' ? max(1, $totalGroups) : $this->perPage;
