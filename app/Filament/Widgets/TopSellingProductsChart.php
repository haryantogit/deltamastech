...
class TopSellingProductsChart extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        if ($grouping === 'category') {
            $query = SalesInvoiceItem::select('products.category_id', DB::raw('SUM(sales_invoice_items.qty) as total_qty'))
            ...
            $query = SalesInvoiceItem::select('sales_invoice_items.product_id', DB::raw('SUM(sales_invoice_items.qty) as total_qty'))
                ->join('sales_invoices', 'sales_invoices.id', '=', 'sales_invoice_items.sales_invoice_id');
        }

        $query->whereBetween('sales_invoices.transaction_date', [$start, $end]);

        $dateLabel = $start->format('d/m/Y') . ' - ' . $end->format('d/m/Y');

        if ($grouping === 'category') {
            $this->heading = "PENJUALAN PER KATEGORI {$dateLabel}";
            ...
            $this->heading = "PENJUALAN PER PRODUK {$dateLabel}";
            $topItems = $query->groupBy('sales_invoice_items.product_id')
                ->orderByDesc('total_qty')
                ->limit(5)
                ->with('product')
                ->get();
    ...
    public function getDescription(): ?string
    {
        ...
        $grouping = $this->filter ?? 'product';
        return "Top 5 " . ($grouping === 'category' ? 'kategori' : 'produk') . " dengan total kuantitas penjualan {$periodLabel}";
    }
