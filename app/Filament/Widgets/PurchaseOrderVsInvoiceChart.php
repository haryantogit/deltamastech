...
class PurchaseOrderVsInvoiceChart extends ChartWidget
{
    protected ?string $heading = 'TAGIHAN & PESANAN (PEMBELIAN)';
    ...
    protected function getData(): array
    {
        ...
        foreach ($months as $date) {
            ...
            $invoiceData[] = PurchaseInvoice::whereMonth('date', $month)
                ->whereYear('date', $year)
                ->sum('total_amount');

            $orderData[] = PurchaseOrder::whereMonth('date', $month)
                ->whereYear('date', $year)
                ->sum('total_amount');
        }
