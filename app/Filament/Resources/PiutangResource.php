...
class PiutangResource extends Resource
{
            ...
            ->schema([
                ...
                Section::make('Rincian')
                    ->schema([
                        ...
                        Hidden::make('status')->default('unpaid'),
                    ]),
            ]);
    }
                        ...
                        Grid::make(2)
                            ->schema([
                                \Filament\Infolists\Components\TextEntry::make('invoice_number')
                                    ...
                                    ->copyable(),
                                \Filament\Infolists\Components\TextEntry::make('contact.name')
                                    ->label('Pelanggan'),
                ...
                Tables\Columns\TextColumn::make('balance')
                    ...
                    ->summarize(
                        Tables\Columns\Summarizers\Summarizer::make()
                            ->label('Total Sisa')
                            ->using(fn($query) => $query->sum(\Illuminate\Support\Facades\DB::raw('total_amount - (select coalesce(sum(amount), 0) from receivable_payments where receivable_id = receivables.id)')))
                    ),
                    ...
                    Action::make('terima_pembayaran')
                        ...
                        ->form([
                            ...
                            TextInput::make('amount')
                                ...
                                ->numeric()
                                ->required()
                                ->default(fn(Receivable $record) => $record->total_amount - $record->payments()->sum('amount')),
                            Textarea::make('notes')
                                ->label('Catatan'),
                            FileUpload::make('attachments')
                                ->label('Bukti Terima / Lampiran')
                                ->multiple()
                                ->directory('receivable-payments')
                                ->downloadable()
                                ->openable(),
                        ])
    ...
    public static function getEloquentQuery(): Builder
    {
        return parent::getEloquentQuery()
            ->where('invoice_number', 'like', 'DM/%');
    }

    public static function getPages(): array
    {
        return [
            'index' => Pages\ListPiutang::route('/'),
