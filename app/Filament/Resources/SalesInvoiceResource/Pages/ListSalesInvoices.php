...
class ListSalesInvoices extends ListRecords
{
    ...
    public function getTabs(): array
    {
        return [
            ...
            'unpaid' => Tab::make('Belum Dibayar')
                ...
                ->badge(SalesInvoice::where('status', 'posted')->where(fn($q) => $q->whereNull('payment_status')->orWhere('payment_status', 'unpaid'))->count()),
            'partial' => Tab::make('Dibayar Sebagian')
                ...
                ->badge(SalesInvoice::where('status', 'posted')->where('payment_status', 'partial')->count()),
            'paid' => Tab::make('Lunas')
                ->modifyQueryUsing(fn(Builder $query) => $query->where('status', 'paid')->orWhere('payment_status', 'paid'))
                ->badge(SalesInvoice::where('status', 'paid')->orWhere('payment_status', 'paid')->count()),
            'overdue' => Tab::make('Jatuh tempo')
                ->modifyQueryUsing(fn(Builder $query) => $query->where('due_date', '<', now())->where('status', 'posted'))
                ->badge(SalesInvoice::where('due_date', '<', now())->where('status', 'posted')->count()),
    ...
    {
        return view('filament.resources.sales-invoice-resource.pages.list-sales-invoices-footer');
    }
