...
class SalesReturnForm
{
                                        ...
                                        ...(request()->has('sales_invoice_id')
                                            ? [
                                                Hidden::make('sales_invoice_id')
                                                    ->default(request()->query('sales_invoice_id'))
                                                    ->dehydrated(),
                                                TextInput::make('sales_invoice_number')
                                                    ->label('Tagihan Penjualan')
                                                    ->default(fn() => SalesInvoice::find(request()->query('sales_invoice_id'))?->invoice_number)
                                                    ->disabled()
                                                    ->dehydrated(),
                                            ]
                                            : [
                                            ...
                                            request()->has('sales_invoice_id')
                                                ? [
                                                    Hidden::make('contact_id')
                                                        ->default(fn() => SalesInvoice::find(request()->query('sales_invoice_id'))?->contact_id)
                                                        ->dehydrated(),
                                                    TextInput::make('contact_name')
                                                        ->label('Pelanggan')
                                                        ->default(fn() => SalesInvoice::with('contact')->find(request()->query('sales_invoice_id'))?->contact?->name)
                                                        ->disabled()
                                                        ->dehydrated(),
                                                ]
                                                : [
                                            ...
                                            request()->has('sales_invoice_id')
                                                ? [
                                                    Hidden::make('warehouse_id')
                                                        ->default(fn() => SalesInvoice::find(request()->query('sales_invoice_id'))?->warehouse_id)
                                                        ->dehydrated(),
                                                    TextInput::make('warehouse_name')
                                                        ->label('Gudang Tujuan')
                                                        ->default(fn() => SalesInvoice::with('warehouse')->find(request()->query('sales_invoice_id'))?->warehouse?->name)
                                                        ->disabled()
                                                        ->dehydrated(),
                                                ]
                                                : [
                        ...
                        Section::make('Item Retur')
                            ->schema([
                                Toggle::make('tax_inclusive')
                                    ...
                                    ->inline(false)
                                    ->default(fn() => (bool) (SalesInvoice::find(request()->query('sales_invoice_id'))?->tax_inclusive ?? false))
                                    ->live()
                                    ->disabled()
                                    ->dehydrated()
                                    ->afterStateUpdated(fn(Set $set, Get $get) => self::updateTotals($get, $set)),
                                                     ...
                                                     Group::make()
                                                        ->schema([
                                                            Repeater::make('items')
                                                                ->relationship()
                                                                ->default(fn() => self::getItemsFromInvoiceRecord(request()->query('sales_invoice_id')))
                                                                ->deletable()
                                                                ->reorderable(false)
                                                                ->schema([
                                                                             Grid::make(12)
    ...
    public static function getItemsFromInvoiceRecord($invoiceId): array
    {
        ...
        $invoice = SalesInvoice::with(['items.product', 'items.unit'])->find($invoiceId);
        if (!$invoice) return [];
        ...
        return $invoice->items()->get()->map(function ($item) {
            return [
                ...
                'invoice_qty' => self::formatNumber($item->qty),
                'returnable_qty' => self::formatNumber($item->qty),
                'return_qty' => 0,
                'unit_price' => self::formatNumber($item->price),
