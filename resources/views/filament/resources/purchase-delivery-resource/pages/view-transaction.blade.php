@php
    ...
    // 1. Delivery Attachments
    if ($record->attachments) {
        foreach ($record->attachments as $att) {
            $allAttachments->push([
                'path' => $att,
                'label' => 'Pengiriman (' . $record->number . ')',
                'source' => 'delivery',
    ...
    // 2. Discover the Transaction Family (searching via PO)
    ...
    if ($purchaseOrder) {
        ...
        if ($purchaseOrder->attachments) {
            foreach ($purchaseOrder->attachments as $att) {
                $allAttachments->push([
                    'path' => $att,
                    'label' => 'Pesanan (' . $purchaseOrder->number . ')',
                    'source' => 'order',
        ...
        // 3. Invoice and Payment Attachments from the family
...
<x-filament-panels::page>
    ...
    <div class="no-print"
        ...
        <div style="display: flex; gap: 8px;">
            <x-filament::button tag="a" href="{{ \App\Filament\Resources\PurchaseDeliveryResource::getUrl('index') }}"
                color="gray" size="sm">
                Kembali
            </x-filament::button>
    ...
    <div class="print-area">
        <div class="po-card">
            ...
            <div class="po-info-grid">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    ...
                    <div>
                        <div class="po-label">Tgl Penerimaan</div>
                        <div class="po-value">{{ $record->date?->format('d/m/Y') ?? '-' }}</div>
                    </div>
                    <div>
                        <div class="po-label">Tgl Pengiriman</div>
                        <div class="po-value">{{ $record->shipping_date?->format('d/m/Y') ?? '-' }}</div>
                    </div>
                    <div>
                        <div class="po-label">Gudang</div>
                        <div class="po-value">
                            {{ $record->warehouse?->name ?? 'Unassigned' }}
                        </div>
                    </div>
                    <div>
                        <div class="po-label">Referensi</div>
                        <div class="po-value">{{ $record->purchaseOrder?->number ?? $record->reference ?? '-' }}</div>
                    </div>
                    <div>
                    ...
                    @foreach($record->items as $item)
                        ...
                        <tr>
                            ...
                            <td style="text-align: center;">
                                @php
                                    $itemStatusLabel = match ($record->status) {
                                        'draft' => 'Draf',
                                        'pending' => 'Menunggu',
                                        'received' => 'Diterima',
                                        'cancelled' => 'Dibatalkan',
                                    ...
                                    $itemStatusColor = match ($record->status) {
                                        'draft' => 'draft',
                                        'pending' => 'pending',
                                        'received' => 'received',
                                        'cancelled' => 'cancelled',
                                ...
                                @endphp
                                <span class="po-badge po-badge-{{ $itemStatusColor }}">{{ $itemStatusLabel }}</span>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
                ...
                <div class="po-summary-grid">
                    ...
                    <div>
                        @if($record->notes)
                            ...
                            <div class="po-value" style="color: #6b7280; white-space: pre-wrap;">{{ $record->notes }}</div>
                        @endif
                    </div>
                    ...
                    <div style="display: flex; justify-content: flex-end;">
                        <div style="width: 320px;">
                            ...
                            <div class="po-summary-row po-summary-total">
                                ...
                                <span
                                    style="font-weight: 700; font-size: 16px;">{{ number_format($record->shipping_cost ?? 0, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="related-transactions-section" style="margin-top: 32px;">
            ...
            <div class="po-card" style="overflow: hidden;">
                @php
                    $purchaseOrder = $record->purchaseOrder;
                    $relatedDeliveries = $purchaseOrder ? $purchaseOrder->deliveries->where('id', '!=', $record->id) : collect();
                @endphp
                    ...
                    <table class="po-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ \App\Filament\Resources\PurchaseOrderResource::getUrl('view', ['record' => $purchaseOrder->id]) }}"
                                        class="po-value-link">{{ $purchaseOrder->number }}</a>
                                </td>
                                ...
                                <td>
                                    ...
                                    @endphp
                                    <span
                                        class="po-badge po-badge-{{ in_array($purchaseOrder->status, ['received', 'paid', 'lunas', 'closed']) ? 'success' : ($purchaseOrder->status === 'cancelled' ? 'cancelled' : 'confirmed') }}">
                                        {{ $poStatusIndo[$purchaseOrder->status] ?? ucfirst($purchaseOrder->status) }}
                                    </span>
                                </td>
                            ...
                            @foreach($purchaseOrder->invoices as $invoice)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ \App\Filament\Resources\PurchaseInvoiceResource::getUrl('view', ['record' => $invoice->id]) }}"
                                            class="po-value-link">{{ $invoice->number }}</a>
                                    </td>
                            ...
                            @if($purchaseOrder->down_payment > 0)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ \App\Filament\Resources\PurchaseOrderResource::getUrl('view', ['record' => $purchaseOrder->id]) }}"
                                            class="po-value-link">
                                            PP/{{ \Illuminate\Support\Str::after($purchaseOrder->number, '/') }}
                                        </a>
                                    </td>
                                    <td>-</td>
                                    <td>
                            ...
                            @foreach($relatedDeliveries as $otherDelivery)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ \App\Filament\Resources\PurchaseDeliveryResource::getUrl('view', ['record' => $otherDelivery->id]) }}"
                                            class="po-value-link">{{ $otherDelivery->number }}</a>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <span
                                            class="po-badge po-badge-{{ $otherDelivery->status === 'received' ? 'success' : ($otherDelivery->status === 'cancelled' ? 'cancelled' : 'confirmed') }}">
                                            {{ $deliveryStatusIndo[$otherDelivery->status] ?? ucfirst($otherDelivery->status) }}
                                        </span>
                                    </td>
                                    <td style="text-align: right; font-weight: 600;">
                                        Rp 0
                ...
                @else
                    <div style="padding: 24px; text-align: center; color: #9ca3af; font-style: italic;">Tidak ada transaksi
                        terkait</div>
                @endif
            </div>
        </div>
