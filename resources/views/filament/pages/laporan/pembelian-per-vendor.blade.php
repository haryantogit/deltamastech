            ...
            <table class="report-table">
                ...
                <tbody>
                    @forelse($vendors as $v)
                        @php $isExpanded = in_array($v['vendor_id'], $expandedVendors); @endphp
                        <tr wire:key="v-{{ $v['vendor_id'] }}" wire:click="toggleVendor({{ $v['vendor_id'] }})"
                            style="cursor: pointer;" class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                            <td>
                                <span class="toggle-btn">{{ $isExpanded ? '−' : '+' }}</span>
                            </td>
                            <td><span class="vendor-link">{{ $v['vendor_name'] }}</span></td>
                            <td>{{ $v['company_name'] ?? '-' }}</td>
                            <td style="text-align: right; font-weight: 500;">{{ number_format($v['transaction_count'], 0) }}
                            </td>
                            <td style="text-align: right; font-weight: 700; color: #3b82f6;">{{ $fmt($v['total_amount']) }}
                            </td>
                        </tr>

                        @if($isExpanded)
                            <tr wire:key="details-{{ $v['vendor_id'] }}">
                                <td colspan="5" style="padding: 0;">
                                    <table class="nested-table">
                                        ...
                                        <tbody>
                                            @foreach($nestedData[$v['vendor_id']] ?? [] as $item)
                                                <tr class="nested-table-row">
                                                    ...
                                                    <td
                                                        style="padding: 0.5rem 1.25rem; font-size: 0.75rem; color: #3b82f6; font-weight: 600;">
                                                        {{ $item['product_name'] }}</td>
                                                    <td
                                                        style="padding: 0.5rem 1.25rem; font-size: 0.75rem; color: #64748b; font-family: monospace;">
                                                        {{ $item['product_sku'] ?? '-' }}</td>
                                                    <td style="padding: 0.5rem 1.25rem; font-size: 0.75rem; color: #64748b;">
                                                    ...
                                                    <td style="padding: 0.5rem 1.25rem; text-align: right; font-size: 0.75rem;">
                                                        {{ $fmt($item['unit_price']) }}</td>
                                                    <td
                                                        style="padding: 0.5rem 1.25rem; text-align: right; font-size: 0.75rem; font-weight: 600; color: #3b82f6;">
                    ...
                    <tr style="border-top: 2px solid rgba(128,128,128,0.2); background: #f8fafc;"
                        ...
                        <td colspan="3" style="padding: 1rem 1.25rem;">Total</td>
                        <td style="text-align: right; padding: 1rem 1.25rem;">{{ number_format($grandTotalCount, 0) }}
                        </td>
