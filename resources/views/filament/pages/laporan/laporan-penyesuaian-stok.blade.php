    ...
    <div class="report-content">
                ...
                <tbody>
                    @forelse($summary as $row)
                        <tr wire:key="row-{{ $row->id }}">
                            ...
                            <td class="number-col">{{ number_format($row->movement_qty, 0, ',', '.') }}</td>
                            <td class="number-col">{{ number_format($row->avg_price, 2, ',', '.') }}</td>
                            <td class="number-col" style="font-weight: 600;">{{ number_format($row->total_value, 2, ',', '.') }}</td>
                        </tr>
                                        ...
                                        <tbody>
                                            @foreach($details[$row->id] ?? [] as $d)
                                                <tr>
                                                    ...
                                                    <td class="number-col">{{ number_format($d->price, 2, ',', '.') }}</td>
                                                    <td class="number-col">{{ number_format($d->total, 2, ',', '.') }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" style="text-align: center; color: #94a3b8; padding: 3rem;">
                                Tidak ada data penyesuaian stok untuk periode ini.
                            </td>
                        </tr>
                    @endforelse
                    ...
                    @if($summary->count() > 0)
                        <tr class="total-row">
                            ...
                            <td class="number-col">{{ number_format($summary->sum('movement_qty'), 0, ',', '.') }}</td>
                            <td class="number-col">-</td>
                            <td class="number-col" style="font-weight: 700;">{{ number_format($data['totalValue'], 2, ',', '.') }}</td>
                        </tr>
                        <tr class="subtotal-highlight">
                            <td colspan="5" style="padding-left: 1rem;">Subtotal Nilai</td>
                            <td class="number-col">{{ number_format($data['totalValue'], 2, ',', '.') }}</td>
                        </tr>
                        <tr class="grand-total-highlight">
                            <td colspan="5" style="padding-left: 1rem;">Total Nilai</td>
                            <td class="number-col">{{ number_format($data['totalValue'], 2, ',', '.') }}</td>
                        </tr>
                    @endif
                </tbody>
        ...
        @if($summary->count() > 0)
            <div style="text-align: right; color: #94a3b8; font-size: 0.75rem; margin-top: 0.5rem;">
                Total {{ $summary->count() }} produk disesuaikan
            </div>
        @endif
    </div>
</x-filament-panels::page>
