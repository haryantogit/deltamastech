<x-filament-panels::page>
    @php
        ...
        extract($viewData);
        $filterLabel = $filterLabel ?? 'vs periode sebelumnya';
        ...
        $trend = function ($current, $previous) use ($filterLabel) {
            $label = $filterLabel ?? 'vs periode sebelumnya';
            if ($previous == 0) {
                $pct = $current != 0 ? 100 : 0;
            } else {
            ...
            return '<span style="font-size:12px;font-weight:600;color:' . $color . '">' . $icon . ' ' . number_format(abs($pct), 1, ',', '.') . '%</span> <span style="font-size:11px;color:#94a3b8;">' . $label . '</span>';
        };

        $trendInline = function ($current, $previous) {
            if ($previous == 0) {
                $pct = $current != 0 ? 100 : 0;
            } else {
                $pct = round((($current - $previous) / abs($previous)) * 100, 1);
            ...
            $icon = $pct >= 0 ? '↑' : '↓';
            $color = $pct >= 0 ? '#22c55e' : '#ef4444';
            return '<span style="font-size:11px;font-weight:600;color:' . $color . '">' . $icon . ' ' . number_format(abs($pct), 1, ',', '.') . '%</span>';
        };
        ...
        $statCard = function ($title, $value, $current, $previous) use ($trend) {
            ...
            return '
                                                                                                                                    <div style="padding:20px;border-radius:12px;border:1px solid rgba(128,128,128,0.15);background:rgba(128,128,128,0.03);">
        ...
        $tdTrend = 'padding:12px 8px;text-align:right;width:90px;';
    @endphp
    ...
    <x-filament::section class="-mt-8">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
            {!! $statCard('Perubahan Modal', $fmt($perubahanModal, 0), $perubahanModal, $prevPerubahanModal) !!}
            {!! $statCard('Saldo Modal', $fmt($saldoModal, 0), $saldoModal, $prevSaldoModal) !!}
            {!! $statCard('Penambahan Modal', $fmt($penambahanModal, 0), $penambahanModal, $prevPenambahanModal) !!}
            {!! $statCard('Pengurangan Modal', $fmt($penguranganModal, 0), $penguranganModal, $prevPenguranganModal) !!}
        </div>
    </x-filament::section>
    ...
    <x-filament::section class="-mt-8">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;">
            ...
            {!! $statCard('Rata-rata Nilai Tagihan', $fmt($avgInvoice, 0), $avgInvoice, $prevAvgInvoice) !!}
    ...
    <x-filament::section class="-mt-8">
        ...
        <table style="width:100%;border-collapse:collapse;">
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdVal }}" class="text-gray-900 dark:text-white">{{ $fmt($kasIn, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($kasIn, $prevKasIn) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                <td style="{{ $tdName }}" class="text-gray-600 dark:text-gray-400">Kas keluar</td>
                <td style="{{ $tdVal }} {{ $valColor(-$kasOut) }}">{{ $fmt($kasOut, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($kasOut, $prevKasOut) !!}</td>
            </tr>
            <tr style="border-top:2px solid rgba(128,128,128,0.2);background:rgba(14,165,233,0.06);">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($kasTotal, $prevKasTotal) !!}</td>
            </tr>
        </table>
    </x-filament::section>
    ...
    <x-filament::section class="-mt-8">
        ...
        <table style="width:100%;border-collapse:collapse;">
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdVal }} {{ $valColor($pendapatan) }}">{{ $fmt($pendapatan, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($pendapatan, $prevPendapatan) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                <td style="{{ $tdName }}" class="text-gray-600 dark:text-gray-400">Biaya penjualan</td>
                <td style="{{ $tdVal }}">{{ $fmt($hpp, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($hpp, $prevHpp) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                <td style="{{ $tdName }};font-weight:700;" class="text-gray-800 dark:text-gray-200">Laba kotor</td>
                <td style="{{ $tdVal }};font-weight:700; {{ $valColor($labaKotor) }}">{{ $fmt($labaKotor, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($labaKotor, $prevLabaKotor) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                <td style="{{ $tdName }}" class="text-gray-600 dark:text-gray-400">Biaya</td>
                <td style="{{ $tdVal }}">{{ $fmt($biaya, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($biaya, $prevBiaya) !!}</td>
            </tr>
            <tr style="border-top:2px solid rgba(217,119,6,0.3);background:rgba(217,119,6,0.06);">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($labaBersih, $prevLabaBersih) !!}</td>
            </tr>
        </table>
    </x-filament::section>
    ...
    <x-filament::section class="-mt-8">
        ...
        <table style="width:100%;border-collapse:collapse;">
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdVal }} {{ $valColor($totalAset) }}">{{ $fmt($totalAset, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($totalAset, $prevTotalAset) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                <td style="{{ $tdName }}" class="text-gray-600 dark:text-gray-400">Liabilitas</td>
                <td style="{{ $tdVal }} {{ $valColor($totalLiabilitas) }}">{{ $fmt($totalLiabilitas, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($totalLiabilitas, $prevTotalLiabilitas) !!}</td>
            </tr>
            <tr style="border-top:2px solid rgba(59,130,246,0.3);background:rgba(59,130,246,0.06);">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($ekuitas, $prevEkuitas) !!}</td>
            </tr>
        </table>
    </x-filament::section>
    ...
    <x-filament::section class="-mt-8">
        ...
        <table style="width:100%;border-collapse:collapse;">
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($jumlahInvoice, $prevJumlahInvoice) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                <td style="{{ $tdName }}" class="text-gray-600 dark:text-gray-400">Rata-rata satu tagihan</td>
                <td style="{{ $tdVal }} {{ $valColor($avgInvoice) }}">{{ $fmt($avgInvoice, 0) }}</td>
                <td style="{{ $tdTrend }}">{!! $trendInline($avgInvoice, $prevAvgInvoice) !!}</td>
            </tr>
        </table>
    </x-filament::section>
    ...
    <x-filament::section class="-mt-8">
        ...
        <table style="width:100%;border-collapse:collapse;">
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($marginLabaKotor, $prevMarginLabaKotor) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($marginLabaBersih, $prevMarginLabaBersih) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($roi, $prevRoi) !!}</td>
            </tr>
        </table>
    </x-filament::section>
    ...
    <x-filament::section class="-mt-8">
        ...
        <table style="width:100%;border-collapse:collapse;">
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($rasioAsetKewajiban, $prevRasioAsetKewajiban) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($rasioHutangEkuitas, $prevRasioHutangEkuitas) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($rasioHutangAset, $prevRasioHutangAset) !!}</td>
            </tr>
            <tr style="{{ $rowStyle }}">
                ...
                <td style="{{ $tdTrend }}">{!! $trendInline($rasioAsetLiabilitas, $prevRasioAsetLiabilitas) !!}</td>
            </tr>
        </table>
    </x-filament::section>
</x-filament-panels::page>