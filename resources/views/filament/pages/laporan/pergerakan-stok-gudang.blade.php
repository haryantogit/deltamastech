...
<x-filament-panels::page>
    <style>
        ...
        /* Tab Filters matching Profitabilitas Produk */
    ...
    <div class="report-content">
        {{-- Warehouse Tabs matching Profitabilitas style --}}
            ...
            @foreach($warehouses as $warehouse)
                <button 
                    ...
                    @class([
                        ...
                        'inactive' => $warehouseId != $warehouse->id,
                    ])
                >
                    ...
                    <tbody>
                        @forelse($summary as $row)
                            <tr wire:key="row-{{ $row->id }}" class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                ...
                                <td class="number-col {{ $row->movement_qty < 0 ? 'text-danger-600 font-semibold' : 'font-semibold' }}">
                                            ...
                                            <tbody>
                                                ...
                                                @foreach($details[$row->id] ?? [] as $m)
                                                    <tr>
                                                        ...
                                                        <td style="color: #64748b;">{{ $m->company }}</td>
                                                        <td class="number-col font-medium">{{ number_format($m->qty_movement, 0, ',', '.') }}</td>
                                                        <td class="number-col">{{ number_format($m->running_qty, 0, ',', '.') }}</td>
                                                        <td class="number-col">{{ number_format($m->price, 0, ',', '.') }}</td>
                                                        <td class="number-col font-medium">{{ number_format($m->total_value, 0, ',', '.') }}</td>
                                                    </tr>
                                                @endforeach
                                                <tr class="saldo-row" style="font-weight: 700;">
                                                    <td colspan="3" style="color: #1e293b;" class="dark:text-gray-300">Saldo Akhir</td>
                            ...
                            <tr>
                                <td colspan="10" style="text-align: center; color: #94a3b8; padding: 4rem;">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <span style="font-size: 0.875rem; font-weight: 500;">Tidak ada data pergerakan stok untuk gudang dan periode ini.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
