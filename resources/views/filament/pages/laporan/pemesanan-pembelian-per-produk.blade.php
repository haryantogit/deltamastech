            ...
            <table class="report-table">
                ...
                <tbody>
                    @forelse($products as $product)
                        @php $isExpanded = in_array($product->product_id, $expandedProducts); @endphp
                        <tr class="row-product" wire:click="toggleProduct({{ $product->product_id }})">
                            <td>
                                <span class="expand-icon">{{ $isExpanded ? '−' : '+' }}</span>
                            </td>
                            <td style="font-weight: 600; color: #3b82f6;">{{ $product->product_name }}</td>
                            <td style="font-family: monospace;">{{ $product->product_sku ?? '-' }}</td>
                            <td>{{ $product->category_name ?? '-' }}</td>
                            <td style="text-align: right;">{{ $fmt($product->current_price) }}</td>
                            <td style="text-align: right;">{{ number_format($product->total_qty, 0, ',', '.') }}</td>
                            <td style="text-align: right; font-weight: 700;">{{ $fmt($product->total_amount) }}</td>
                            <td style="text-align: right;">
                                {{ $fmt($product->total_qty > 0 ? $product->total_amount / $product->total_qty : 0) }}</td>
                        </tr>
                                ...
                                <td colspan="8" style="padding: 0;">
                                    <div class="details-wrapper">
                                        <table class="details-table">
                                            ...
                                            <tbody>
                                                @forelse($nestedData[$product->product_id] ?? [] as $item)
                                                    <tr>
                                                        <td style="font-weight: 600; color: #3b82f6;">{{ $item->order_number }}</td>
                                                ...
                                                @empty
                                                    <tr>
                                                        <td colspan="5" style="text-align: center; color: #94a3b8;">Tidak ada data
                                                            detail.</td>
                                                    </tr>
                                                @endforelse
                                            </tbody>
                ...
                <tfoot>
                    <tr class="total-row">
                        ...
                        <td style="text-align: right;">
                            {{ $fmt($grandTotalQty > 0 ? $grandTotalAmount / $grandTotalQty : 0) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
